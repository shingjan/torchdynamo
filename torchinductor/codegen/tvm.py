...
class TIROverrides(OpOverrides):
    ...
    @staticmethod
    def rsqrt(x):
        return f"T.libdevice.rsqrt({x})"

    @staticmethod
    def signbit(x):
        # XX: This is wrong for the value -0.0 in floating point
        return f"T.libdevice.signbitf({x}) if {x}.dtype is T.float32 else {x} < 0"

    @staticmethod
    def fmod(a, b):
        return f"T.libdevice.fmod({a}, {b})"

    @staticmethod
    def pow(a, b):
        return f"T.libdevice.pow({a}, {b})"
...
class IterationRangesRoot(IterationRanges):
    def __init__(
        ...
        # pid_cache["T.program_id(0)"] = pid_m
    ...
    def ranges_code(self):
        ...
        return f"T.reshape(T.arange(0, {self.prefix.upper()}BLOCK), {size})"
...
class TIRKernel(Kernel):
    ...
    def indexing(
        ...
        """
        Compute the index and mask to pass to T.load() or T.store()
        """
        ...
        if (need_dense and not have_dense) or index == 0:
            index_str = f"{index_str} + T.zeros({self.dense_size_str()}, T.int32)"
            if index == 0:
            ...
            mask = dense_mask
            index_str = f"{index_str} + T.zeros({copy_shape}.shape, T.int32)"
    ...
    def mask_loads(self, mask):
        """Context manager to add an additional mask to T.load/store"""
    ...
    def reduction(self, name, dtype, src_dtype, reduction_type, index, value):
        ...
        if (src_dtype, reduction_type, value) not in self.cse.reduction_cache:
            ...
            if reduction_type in {"argmax", "argmin"}:
                ...
                self.body.writeline(
                    f"{accumulator_index} = T.zeros({self.dense_size_str()}, T.int64)"
                )
            ...
            if accumulator_index:
                ...
                self.compute.writeline(
                    f"{accumulator_index} = T.where({cond},  {reduction_range_prefix}index, {accumulator_index})",
                )
            self.compute.writeline(
                f"{accumulator} = T.where({cond}, {updated}, {accumulator})"
            )
            ...
            if accumulator_index:
                ...
                self.suffix.writelines(
                    [
                        f"{accumulator_index}_reduce = T.reshape(",
                        f"\ttl.{reduction_type}({accumulator}, {dim}), [{', '.join(sizes)}]).to(T.int32)",
                        f"{accumulator_index}_mask = (T.reshape(T.arange(0, {reduction_range_prefix.upper()}BLOCK),",
                        f"\t[{', '.join(reduction_sizes)}]) == {accumulator_index}_reduce)",
                        f"{result_var} = T.reshape(T.sum(",
                        f"\ttl.where({accumulator_index}_mask, {accumulator_index}, 0), {dim}), [{', '.join(sizes)}])",
                    ]
                )
            else:
                self.suffix.writeline(
                    f"{result_var} = T.reshape(T.{reduction_type}({accumulator}, {dim}), [{', '.join(sizes)}])"
                )
        else:
        ...
        if name not in V.graph.removed_buffers:
            var = self.args.output(name)
            self.suffix.writeline(
                DeferredLine(name, f"T.store({var} + {index}, {result_var}, {mask})")
            )
...
class TIRScheduling:
    ...
    def can_fuse(self, node1, node2):
        """
        Hook called by Scheduler to determine if the TIR backend
        ...
        if not node1.is_reduction() and node2.is_reduction():
            ...
            if numel1 == numel2 * rnumel2:
                if not all(
                    TIRKernel.is_compatible((numel2, rnumel2), n.get_ranges())
    ...
    def codegen_node_schedule(self, node_schedule, numel, reduction_numel):
        ...
        if len(reductions) > 0:
            ...
            reduction_hint_val = ReductionHint.DEFAULT
        with TIRKernel(*tiled_groups, reduction_hint=reduction_hint_val) as kernel:
    ...
    def select_tiling(cls, node_schedule, numel, reduction_numel=sympy.Integer(1)):
        ...
        for tiled_groups in ranked_tilings:
            ...
            if all(
                TIRKernel.is_compatible(new_groups, node.get_ranges())
                for node in node_schedule
